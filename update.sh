...
# hypr: user lua files only, never noctalia.lua (generated by the Noctalia hyprland template)
for f in "$HOME"/.config/hypr/*.lua; do
  [ "$(basename "$f")" = "noctalia.lua" ] && continue
  cp -f "$f" "$REPO_DIR/dotfiles/hypr/.config/hypr/"
done
for f in "$REPO_DIR"/dotfiles/hypr/.config/hypr/*.lua; do
  [ -f "$HOME/.config/hypr/$(basename "$f")" ] || rm -f "$f"
done
